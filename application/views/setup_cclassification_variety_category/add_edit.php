...
<form class="form_valid" id="save_form" action="<?php echo site_url($CI->controller_url.'/index/save');?>" method="post">
    <input type="hidden" id="id" name="id" value="<?php echo $variety_category['id']; ?>" />
    ...
    <div class="row widget">
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="variety_category[name]" id="name" class="form-control" value="<?php echo $variety_category['name'];?>"/>
            </div>
        </div>
        <div class="row show-grid">
            <div class="col-xs-4">
            ...
            <div class="col-sm-4 col-xs-8">
                <textarea name="variety_category[description]" class="form-control"><?php echo $variety_category['description']; ?></textarea>
            </div>
        </div>
        <div style="" class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="variety_category[ordering]" id="ordering" class="form-control" value="<?php echo $variety_category['ordering'] ?>" >
            </div>
        </div>
        <div style="" class="row show-grid">
            <div class="col-xs-4">
            ...
            <div class="col-sm-4 col-xs-8">
                <select id="status" name="variety_category[status]" class="form-control">
                    <!--<option value=""></option>-->
                    <option value="<?php echo $CI->config->item('system_status_active'); ?>"
                        <?php
                        if ($variety_category['status'] == $CI->config->item('system_status_active')) {
                    ...
                    <option value="<?php echo $CI->config->item('system_status_inactive'); ?>"
                        <?php
                        if ($variety_category['status'] == $CI->config->item('system_status_inactive')) {
                            echo "selected='selected'";
                        }
