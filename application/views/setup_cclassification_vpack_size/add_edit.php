...
<form class="form_valid" id="save_form" action="<?php echo site_url($CI->controller_url.'/index/save');?>" method="post">
    <input type="hidden" id="id" name="id" value="<?php echo $pack['id']; ?>" />
    ...
    <div class="row widget">
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="pack[name]" id="name" class="form-control" value="<?php echo $pack['name'];?>"/>
            </div>
        </div>
        <div style="" class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <textarea name="pack[description]" id="description" class="form-control"><?php echo $pack['description'] ?></textarea>
            </div>
        </div>
        <div style="" class="row show-grid">
            <div class="col-xs-4">
            ...
            <div class="col-sm-4 col-xs-8">
                <select id="status" name="pack[status]" class="form-control">
                    <!--<option value=""></option>-->
                    <option value="<?php echo $CI->config->item('system_status_active'); ?>"
                        <?php
                        if ($pack['status'] == $CI->config->item('system_status_active')) {
                    ...
                    <option value="<?php echo $CI->config->item('system_status_inactive'); ?>"
                        <?php
                        if ($pack['status'] == $CI->config->item('system_status_inactive')) {
                            echo "selected='selected'";
                        }
