...
<form class="form_valid" id="save_form" action="<?php echo site_url($CI->controller_url.'/index/save');?>" method="post">
    <input type="hidden" id="id" name="id" value="<?php echo $competitor['id']; ?>" />
    ...
    <div class="row widget">
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="competitor[name]" id="name" class="form-control" value="<?php echo $competitor['name'];?>"/>
            </div>
        </div>
        <div class="row show-grid">
            <div class="col-xs-4">
            ...
            <div class="col-sm-4 col-xs-8">
                <textarea name="competitor[description]" class="form-control"><?php echo $competitor['description']; ?></textarea>
            </div>
        </div>
        <div style="" class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="competitor[ordering]" id="ordering" class="form-control" value="<?php echo $competitor['ordering'] ?>" >
            </div>
        </div>
        <div style="" class="row show-grid">
            <div class="col-xs-4">
            ...
            <div class="col-sm-4 col-xs-8">
                <select id="status" name="competitor[status]" class="form-control">
                    <!--<option value=""></option>-->
                    <option value="<?php echo $CI->config->item('system_status_active'); ?>"
                        <?php
                        if ($competitor['status'] == $CI->config->item('system_status_active')) {
                    ...
                    <option value="<?php echo $CI->config->item('system_status_inactive'); ?>"
                        <?php
                        if ($competitor['status'] == $CI->config->item('system_status_inactive')) {
                            echo "selected='selected'";
                        }
